...
> **Version**: 1.2
> **Date**: 2026-03-04
> **Related**: [`canonical-projection-model.md`](canonical-projection-model.md) (개념적 기반), [`judgment-driven-development.md`](judgment-driven-development.md) (설계 철학), [`translation-ontology.md`](translation-ontology.md) (통합 프레이밍), [`reviews/lld-gap-analysis-and-implementation-plan.md`](reviews/lld-gap-analysis-and-implementation-plan.md) (구현 계획)
...
> **참고**: "두 가지 Grammar" 모델은 operational shorthand로 유효하나, 주요 개념적 프레임은 Canonical + 3 Projections 모델이다. 전체 모델은 [`canonical-projection-model.md`](canonical-projection-model.md) 참조.
...
> **이 섹션은 아카이브되었습니다.** 수학적 프레이밍(미적분학, 사영기하학, 제약 최적화)은 Canonical + 3 Projections 모델로 대체되었습니다.
>
> - **대체 문서**: [`canonical-projection-model.md`](canonical-projection-model.md) — Convergence, Carry-Forward as Projection Gaps, Translation Reframed
> - **아카이브된 개념의 상세**: [`deprecated-concepts.md`](deprecated-concepts.md)
> - **이 섹션의 원래 내용**: DDD v1.1 §5.1-5.5
>
> 아래 개념 매핑을 참조하십시오:
>
> | 이전 (v1.1) | 현재 (v1.2) | 참조 |
> |---|---|---|
> | 미분/적분 | Convergence (gap-measure-iterate-zero) | canonical-projection-model.md §3 |
> | 적분 상수 (Carry-Forward) | Projection Gaps | canonical-projection-model.md §4 |
> | π 표기법 N-사영 | 3 Named Projections (Code, Policy, Experience) | canonical-projection-model.md §1 |
> | 제약 최적화 | JP conflict resolution | JDD + pipeline |
> | 2-Grammar 모델 | 3-Projection 모델 (2-Grammar는 operational shorthand) | canonical-projection-model.md §2 |
...
세 입력 중 하나라도 빠지면 번역은 불완전하다. Prototype 없이는 번역 대상이 없고, Carry-Forward 없이는 Policy/Code projection gap이 빠지며 (`canonical-projection-model.md` §4 참조), Brownfield 없이는 Delta를 계산할 수 없다.
