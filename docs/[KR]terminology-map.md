...
| Korean | English (Canonical) | Context |
|--------|-------------------|---------|
| 사용자 문법 | user grammar | Experience projection의 operational shorthand. 시스템의 실제 사용자가 쓰는 언어 |
| 개발 문법 | development grammar | Code projection의 operational shorthand. 구현의 언어 (API, DB, 상태 머신) |
...
| 캐리포워드 | carry-forward | Experience projection에 대응물이 없는 Code/Policy projection 요소. projection gap이 구조적 원인 |
...
| 사영 | projection | DEPRECATED — `deprecated-concepts.md` 참조. 수학적 프레이밍: π 표기법 N-사영 |
| 제약 최적화 | constrained optimization | DEPRECATED — `deprecated-concepts.md` 참조. 수학적 프레이밍: 고객 만족(목적 함수) + 제약 조건 |
| 목적 함수 | objective function | DEPRECATED — `deprecated-concepts.md` 참조. 수학적 프레이밍: 고객(주 사용자) 사영 |
| 제약 조건 | constraints | DEPRECATED — `deprecated-concepts.md` 참조. 수학적 프레이밍: 서비스 제공자 사영 (NFR 등) |
| 경계 조건 | boundary conditions | DEPRECATED — `deprecated-concepts.md` 참조. 수학적 프레이밍: 적분 상수를 결정하는 조건 |
| 실현 가능 영역 | feasible region | DEPRECATED — `deprecated-concepts.md` 참조. 수학적 프레이밍: 모든 제약을 만족하는 해 집합 |
| 미적분의 기본 정리 | fundamental theorem of calculus | DEPRECATED — `deprecated-concepts.md` 참조. 수학적 프레이밍: 왕복 검증의 수학적 대응물 |
| 정규 정의 | canonical definition | 서비스의 완전한 내부 일관적 정의. 직접 관측 불가 — projection을 통해서만 관측됨 |
| 투영 | projection | 정규 정의의 명명된 부분 뷰. Code, Policy, Experience 세 가지 |
| 수렴 | convergence | 파이프라인 각 단계의 작동 원리: gap 측정 → 반복 → zero까지 |
| 투영 간극 | projection gap | 한 projection에 존재하나 다른 projection에는 대응물이 없는 정보. carry-forward의 구조적 원인 |
| 정규 투영 모델 | canonical projection model | Sprint Kit의 서비스 모델링 방법: 정규 정의 + 3 projection |
