...
> **Version**: 1.0
> **Date**: 2026-02-22
> **관련 문서**: [`canonical-projection-model.md`](canonical-projection-model.md) (개념적 기반), [`judgment-driven-development.md`](judgment-driven-development.md) (설계 철학), [`delta-driven-design.md`](delta-driven-design.md) (설계 이론), [`blueprint.md`](blueprint.md) (제품 명세), [`terminology-map.md`](terminology-map.md) (용어 참조)
...
| 부분 | 성격 | 본질 |
|------|------|------|
| **A1 — Ontological(존재론적)**: "핵심 작동은 번역이다" | Sprint Kit이 *하는 것* | Experience projection과 Code projection 사이에 규칙 기반 매핑이 존재하며, Crystallize가 이 매핑을 실행한다 |
...
| ID | 가설 | 근거 | 파괴 조건 |
|----|------|------|-----------|
| **H1** | 세 projection(Experience, Code, Policy)은 분리 가능하다 | 시스템의 사용자, 구현자, 규제자가 서로 다른 표현 체계를 사용한다 | projection이 하나로 축소되는 경우 — 예: 1인 개발자가 개인 도구를 만드는 경우(사용자 = 개발자 = 정책결정자: 세 projection 모두 합쳐짐) 또는 규정이 UX를 완전히 지배하는 시스템(Policy ≈ Experience) |
...
| **H4** | 번역 과정에서 손실된 정보(carry-forward)는 별도 소스에서 복구할 수 있다 | NFR, 보안, 마이그레이션 등은 PRD, Architecture, Brownfield에서 주입된다 | carry-forward 소스가 불완전하거나 접근 불가능한 경우 — 예: PRD나 아키텍처 문서가 없는 레거시 시스템, 또는 코드베이스에 접근할 수 없는 서드파티 서비스. Sprint Kit은 degradation: 검증 불가능한 carry-forward 항목은 UNKNOWN provenance로 플래그됨 |
...
```
A1: 핵심 작동은 번역이다
  + H1: 세 projection은 분리 가능하다
  + H2: 결정론적 매핑 규칙이 존재한다
    → 3-Projection Model: Experience ↔ Code ↔ Policy
      + H4: 손실된 정보는 별도 소스에서 복구 가능하다
        → FP6: 번역은 규칙 기반이다
          → Complete Specs = translate(Experience_to_be, CP) + carry-forward(Policy, Code_as_is)
...
- carry-forward 없이: projection gap이 누락된다 — 비가시적 요구사항이 손실된다 (`canonical-projection-model.md` §4 참조) (비가시적 요구사항이 손실된다)
...
### 7.2 Projection 수 문제 — Canonical + 3 Projections로 해결됨

이전 버전(v1.0)에서 "2개인가 N개인가?"라는 질문이 미해결 상태였다. [`canonical-projection-model.md`](canonical-projection-model.md)가 이를 해결했다:

- **3개**: Experience, Code, Policy. Sprint Kit이 실제로 구분하는 projection은 정확히 3개이다.
- **2-Grammar는 operational shorthand**: User Grammar = Experience projection, Development Grammar = Code projection. 번역 규칙에서 계속 사용되는 운용 약칭이다.
- **N-projection은 불필요**: π 표기법의 무한 확장은 실행 가능한 구분을 제공하지 못했다. 3개로 고정함으로써 각 projection에 구체적 산출물과 소스 역할이 매핑된다.
- **Policy가 독립 projection**: 이전에 Development Grammar에 암묵적으로 포함되었던 정책/규제 제약이 명시적 projection으로 분리되었다.

상세: [`canonical-projection-model.md`](canonical-projection-model.md) §2 "Why Three Projections (Not Two)".
