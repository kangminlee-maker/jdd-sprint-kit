...
S2 is skipped when brownfield-context.md has no Constraint Profile section, or when all prototype concepts are already covered (delta=0). PCP (Policy Constraint Profile) checking runs as an inline Conductor check before S3 agents, independent of Agent A's CP-based skip condition — this ensures policy conflicts are caught even when Agent A is skipped. S3 Agent A is skipped when no HIGH confidence CP items exist; Agent B always runs. S3-R (Resolution Phase) runs when S3 or PCP check produces findings. S3 findings are classified as HARD_CONFLICT (DB-enforced constraints causing runtime failure — auto-resolved, displayed individually), DECISION_REQUIRED (soft constraints and policy conflicts — user decides), or PROTOTYPE_GAP (structural gaps — spec-level workaround options using carry-forward taxonomy). S3 findings are classified into three resolution types that determine how they are processed:
- **HARD_CONFLICT**: DB-enforced constraints where violation causes runtime failure (NOT NULL violation, FK integrity error, type-cast data loss, cascading constraint violation). These are auto-resolved — only one valid resolution exists. Displayed individually with full context.
- **DECISION_REQUIRED**: All other constraint findings plus policy conflicts (PCP). These are presented to the product expert with options in customer-visible language.
- **PROTOTYPE_GAP**: Structural gaps in the prototype where spec-level workarounds exist. Presented with carry-forward taxonomy options (add as new requirement, defer, or acknowledge as known gap).

All CP patterns are treated as **soft constraints** (observed patterns, not absolute rules) — only DB-enforced rules qualify as hard conflicts. Questions to the product expert use customer-visible language (screens, buttons, behaviors); database/API terminology appears only in collapsible technical details (the "UX-Language Questions" principle).

When Agent B finds PRD requirements absent from the prototype, it classifies them into 4 categories: INVISIBLE (inherently non-visible like security/monitoring), ACCESS_GATED (role-specific), OUT_OF_SCOPE (removed by JP2 decisions), MISSING (should be visible but absent). MISSING items become DECISION_REQUIRED; the rest carry forward automatically via the carry-forward registry.
...
| **HARD_CONFLICT** | S3 resolution type. DB-enforced constraints where violation causes runtime failure (NOT NULL, FK integrity, type-cast data loss, cascading). Auto-resolved — only one valid answer exists |
| **DECISION_REQUIRED** | S3 resolution type. All constraint findings that are not HARD_CONFLICT or PROTOTYPE_GAP, plus PCP policy conflicts. Presented to the product expert with options |
| **PROTOTYPE_GAP** | S3 resolution type. Structural gaps in the prototype where spec-level workarounds exist. Presented with carry-forward taxonomy options |
| **Soft Constraint Principle** | All Constraint Profile items are soft constraints (observed patterns, not absolute rules). Only DB-enforced rules are hard constraints. Governs S3 finding classification |
| **UX Question Template** | S3-R presentation format. Customer-visible language in main text; database/API terms only in collapsible `<details>` blocks. Ensures product experts can decide without understanding technical implementation |
