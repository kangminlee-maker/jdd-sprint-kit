...
> 근본 관점과 전제 체계: [`docs/translation-ontology.md`](translation-ontology.md)
...
| **Sprint Kit** | BMad Method를 활용하는 실행 레이어: 자동 파이프라인, Specs, Deliverables, 프로토타입 |
...
시스템 내부 최적화에는 캐리포워드와 Brownfield 수집 비용을 포함하는 5항 공식이 사용됩니다:

```
C_total = C_input + C_gen × N_gen + C_judge × N_judge + C_carry + C_brownfield
```

여기서 C_carry는 비가시 요구사항(NFR, 보안, 마이그레이션) 등록/검증 비용이며, C_brownfield는 기존 시스템 맥락 수집/파싱 비용입니다. 사용자에게는 위의 3항 공식으로 충분합니다.
...
이상적으로는 Brief 입력 후 사용자가 바로 프로토타입(JP2)으로 가서 최종 산출물만 판단하면 된다. 그러나 JP1에는 JP2가 대체할 수 없는 **방향 검증(direction validation)** 고유 기능이 있다: 프로토타입에 없는 시나리오의 누락 감지, 요구사항 수준의 우선순위 판단, 고객 여정과의 정합성 확인은 JP2에서 수행할 수 없다. 또한 현재 AI 속도에서는 한 번에 JP2에 도달하는 데 수십 분이 걸리므로, JP1은 방향 검증이라는 고유 역할과 실용적 보완을 동시에 수행한다.

AI가 충분히 빨라져도 JP1의 형태는 변할 수 있으나(예: 프로토타입과 함께 요구사항 체크리스트 제시), 방향 검증 기능 자체는 남을 수 있다.
...
- **Sprint Kit은 별도의 시스템이 아니다** — BMad Method를 활용하며, 현재 BMad에 종속되어 있다. "두 시스템 간의 브릿지" 개념은 필요 없다. 연결점은 파일 포맷 계약(planning-artifacts/ 디렉토리)이다.
...
| BMad 산출물 포맷 의존 | BMad 버전 변경 시 호환성 문제 | BMad 종속 — 분리 로드맵은 [`reviews/translation-ontology-roadmap.md`](reviews/translation-ontology-roadmap.md) 참조 | 도구 스택 (S4.1) |
