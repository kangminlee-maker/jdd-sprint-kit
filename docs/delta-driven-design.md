...
제약이 충돌하는 경우 — 예를 들어, 최상의 고객 경험이 보안 요구사항을 위반하는 경우 — 실현 가능 영역이 비어있게 된다. 이때 어떤 제약을 완화할지는 기계적으로 결정할 수 없다. **이것이 CP1("사람의 판단만이 영속 자산")이 필수적인 이유다** — 제약 충돌의 해소는 비즈니스 판단이며, JP가 그 판단 시점이다.
