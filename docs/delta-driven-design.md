...
델타 추출은 미분이다. 변화량을 추출하는 연산이다:

```
f'(x) ↔ Delta 추출
```

완전한 시스템 복원은 적분이다. 델타로부터 원래 시스템을 복원하는 연산이다:

```
∫f'(x)dx + C ↔ Brownfield + Delta + Carry-Forward = Complete System
```

| 미적분 | Delta-Driven Design |
|---|---|
| f(x) — 원래 함수 | Complete System — 목표 시스템 |
| f'(x) — 도함수 | Delta — 변화량 |
| ∫f'(x)dx — 부정적분 | Brownfield + Delta — 가시적 변경의 합 |
| C — 적분상수 | Carry-Forward — 비가시 요구사항 |
| 초기/경계 조건으로 C 결정 | PRD/Architecture/Brownfield에서 Carry-Forward 결정 |

### 적분상수로서의 캐리포워드

미분에서 상수의 도함수는 0이다. 아무리 미분해도 상수 항은 사라진다. 이와 정확히 같은 이유로 캐리포워드 항목은 프로토타입에서 보이지 않는다:

| 미적분 | Delta-Driven Design |
|---|---|
| d/dx[C] = 0 | NFR/보안은 프로토타입에서 보이지 않음 |
| 적분만으로는 C 복원 불가 | 프로토타입 번역만으로는 비가시 요구사항 복원 불가 |
| 초기 조건 f(x₀) = y₀로 C 결정 | PRD, Architecture, Brownfield에서 캐리포워드 결정 |
| C를 빠뜨리면 해가 불완전 | 캐리포워드를 빠뜨리면 시스템이 불완전 |

이것이 Crystallize에서 캐리포워드가 필수인 이유다. 프로토타입 번역(부정적분)만으로는 시스템이 완전해질 수 없다 — 초기/경계 조건(PRD, Architecture)에서 결정되는 적분상수(캐리포워드)가 반드시 추가되어야 한다.

### 왕복 검증 = 미적분의 기본 정리

미적분의 기본 정리는 미분과 적분이 역연산임을 보장한다:

```
∫(d/dx[f(x)])dx + C = f(x)
```

Delta-Driven Design의 왕복 검증은 이것과 동일한 구조다:

```
translate(Prototype) → Specs → re-generate → Re-Prototype ≈ Prototype
```

미분(델타 추출) 후 적분(시스템 복원)하면 원래 함수(프로토타입)로 돌아와야 한다. 돌아오지 않으면 번역에서 정보가 손실된 것이다 — 이는 적분상수(캐리포워드)가 누락되었거나 미분(번역 규칙)이 불완전했음을 의미한다.

### 사영으로서의 프로토타입

완전한 시스템 S는 고차원 객체다 — API, DB, 보안, 성능, 모니터링, UI, 비즈니스 로직 등 수많은 차원을 가진다. 프로토타입은 이 고차원 시스템을 특정 관점의 저차원으로 사영(projection)한 것이다:

```
π_customer(S) = 고객이 보는 시스템 (UI, 인터랙션, 흐름)
π_developer(S) = 개발자가 보는 시스템 (API, DB, 상태 머신)
π_security(S) = 보안 팀이 보는 시스템 (인증, 인가, 암호화)
π_ops(S) = 운영 팀이 보는 시스템 (배포, 모니터링, 스케일링)
```

각 사영은 서로 다른 정보를 잃는다. 고객 사영(π_customer)에서는 보안 규칙이 보이지 않고, 보안 사영(π_security)에서는 UI 레이아웃이 보이지 않는다. 캐리포워드란 **특정 사영에서 소실된 차원의 정보**다.

CT 스캔이 여러 각도의 사영을 결합하여 원래 3D 구조를 재구성하듯, Sprint Kit은 여러 관점의 산출물(프로토타입, PRD, Architecture, Brownfield)을 결합하여 완전한 시스템 명세를 재구성한다.

### 사영의 위계: 제약 최적화

**모든 사영이 평등하지는 않다.** 이것은 2문법 모델에 대한 핵심 보정이다.

고객(primary user) 사영은 **목적 함수**다 — 최적화해야 할 대상이다. 나머지 사영(개발, 보안, 운영)은 **제약 조건**이다 — 만족해야 하지만 최적화 대상은 아니다.
...
이 구조는 Sprint Kit의 현재 설계와 정확히 대응한다:

| 수학적 프레이밍 | Sprint Kit 대응 |
|---|---|
| 목적 함수 (π_customer) | FR — 기능 요구사항 |
| 제약 조건 (π_security, π_ops, ...) | NFR — 비기능 요구사항 |
| 실현 가능 영역 (모든 제약을 만족하는 해 집합) | 모든 NFR을 만족하면서 FR을 구현하는 설계 |
| 실현 가능 영역이 비어있음 (제약 충돌) | NFR 간 충돌 → 비즈니스 판단 필요 |
| 최적해 선택 | JP1/JP2에서의 판단 |

제약이 충돌하는 경우 — 예를 들어, 최상의 고객 경험이 보안 요구사항을 위반하는 경우 — 실현 가능 영역이 비어있게 된다. 이때 어떤 제약을 완화할지는 기계적으로 결정할 수 없다. **이것이 FP1("사람의 판단만이 영속 자산")이 필수적인 이유다** — 제약 충돌의 해소는 비즈니스 판단이며, JP가 그 판단 시점이다.

### 시사점

이 수학적 프레이밍은 새로운 것을 추가하지 않는다 — **현재 파이프라인이 이미 이 구조를 반영하고 있음**을 드러낸다.

- Crystallize는 미분(델타 추출) + 적분상수 주입(캐리포워드)을 수행하고 있었다
- 왕복 검증은 미적분의 기본 정리를 적용하고 있었다
- 프로토타입은 이미 고객 사영이었다
- PRD의 FR/NFR 구분은 이미 목적 함수/제약 조건의 구조였다
- JP에서의 판단은 이미 제약 충돌의 해소였다

또한, **2문법 모델은 위계가 있는 N-사영 모델의 특수 사례**임이 드러난다. User Grammar는 목적 함수인 고객 사영이고, Development Grammar는 나머지 제약 조건 사영들을 하나로 묶은 것이다. 사영의 수를 2개에서 N개로 확장해도 파이프라인의 구조는 변하지 않는다 — 각 사영에 대해 캐리포워드와 검증이 필요할 뿐이다.

---

## 6. Sprint Kit 파이프라인 재구성
...
이것은 Crystallize S7 (교차 산출물 일관성) 검사를 번역 검증 단계로 재구성한 것이다. §5의 수학적 프레이밍으로 보면, 이것은 미적분의 기본 정리와 동일한 구조다.
...
#### FP2: 프로토타입은 시스템의 실제 사용자에 맞추어 적응한다 (신규)
...
#### FP3: 명세 완전성이 비결정성을 통제하고, 우아한 퇴화가 나머지를 관리한다 (신규)
...
#### FP4: 델타 정의가 Sprint Kit의 주요 목표다 (신규)
...
Sprint Kit이 비결정성을 수용하는 곳 (예: UI 레이아웃 변동)에서, 이는 특정 우아한 퇴화 전략을 갖는 문서화된 설계 선택이다 — 인정되지 않은 갭이 아니다. 각 영역의 명세 완전성 수준은 명시적으로 기술된다 (FP3 표 참조).
...
| 요소 | 유지 이유 |
|---|---|
| **FP1: 사람의 판단만이 영속 자산이다** | JDD의 기반. 델타 재구성에 의해 변경되지 않음 |
...
| **Contract-First (Specmatic)** | 계약이 실행 중 비결정성을 통제 (FP3 전략) |
...
| 라운드 8 | 설계 원칙 확정 | FP1-FP6 (핵심 원칙) + DJ1-DJ10 (설계 판단). 주요 추가: FP2 (프로토타입이 사용자 유형에 적응), FP3 (명세 완전성 ↔ 비결정성 트레이드오프), FP4 (델타가 주요 목표). 옵션 B 선택 |
| 라운드 9 | 수학적 프레이밍 | 미적분/사영/제약 최적화 비유. 캐리포워드 = 적분상수, 프로토타입 = 사영, 구현 = 제약 최적화. 2문법 모델이 위계적 N-사영의 특수 사례임을 확인 |
