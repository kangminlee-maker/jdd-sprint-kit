...
- 에이전트는 `tracking_source` 필드만 읽고 분기
...
**예외**: `force_cp1_review` 필드명은 유지합니다 (설계 결정 #5).

### 수정 2: force_cp1_review 주석 처리

Step 0 (L66~69)의 `force_cp1_review` 관련 내용을 다음으로 교체합니다:
...
Step 4a (L391~486) 전체를 다음으로 교체합니다:

```markdown
#### Step 4a: 정보 배너 + Visual Summary 생성

산출물에서 **메타데이터만** 추출하여 JP1 시각화를 생성합니다. 전문 읽기 금지 — Conductor 원칙 유지.
...
readiness.md에서 다음 데이터를 추출하여 배너를 생성합니다:
...
**정보 배너 다음에 항상 풀 Visual Summary를 표시합니다.**
```

### 수정 4: JP1 Visual Summary 재설계

기존 4-Section (L420~486)을 다음 3-Section + Advanced 구조로 교체합니다:
...
**중요 — Section 1~3만 출력합니다.** 아래 Advanced 항목은 절대 기본 표시하지 않습니다.
...
Step 6 피드백 라우팅 테이블 (L738~743)에 행을 추가합니다:

```markdown
### Judgment Point 2 피드백 (Sprint 산출물 단계)
...
Step 6b A/P/C 메뉴의 R(Redirect) 설명에 추가합니다:
...
| 고객 영향 열 | 출력 형식에 추가 | ~8줄 |

### 수정 1: goals 빈 배열 fallback (Stage 1 앞에 삽입)

L26 (Stage 1 제목) 앞에 삽입합니다:
...
1. `artifact_path`에서 PRD 경로를 추론합니다:
...
2. PRD의 **Success Criteria > Measurable Outcomes** 섹션에서 목표 3~5개를 추출합니다
3. 추출한 goals를 Stage 1 커버리지 매핑에 사용합니다
4. sprint-input.md에 역기록하지 않습니다 (Scope Gate 내부에서만 사용)

**적용 범위**: goals fallback은 **PRD 이후 단계**(prd, architecture, epics, spec)에서만 적용합니다.
product-brief 단계에서 goals가 비어있으면 Stage 1을 SKIP하고 Stage 2-3만 실행합니다.
```

### 수정 2: 고객 영향 열 (출력 형식 보완)

Stage 1 Structured Probe 테이블 (L30~35)에 열을 추가합니다:

```markdown
| Goal | Covered By | Section/Line | 고객 영향 | Status |
|------|-----------|--------------|----------|--------|
| {goal_1} | {specific item} | {section} | {고객에게 미치는 영향 1줄} | COVERED / UNCOVERED |
```

Stage 3 Holistic Review (L110~117)에 항목을 추가합니다:

```markdown
- **고객 관점 공백**: 고객 관점에서 빠진 시나리오나 사용자 경험 단절
```

출력 형식의 Overall Verdict (L152~163) 뒤에 추가합니다:
...
| readiness.md JP1 데이터 | 자체 검증 확장 + specs-only 모드 | ~25줄 |
| CP→JP 소수 치환 | L288 등 | ~3곳 |

### 수정 1: tracking_source 기반 분기 (Stage 2 시작에 삽입)

Stage 2 (L50) 시작 부분에 삽입합니다:
...
2. 존재하면 `tracking_source` 필드를 읽습니다
3. 미존재하면 `tracking_source: success-criteria`로 간주합니다
...
자체 검증 섹션 (L278~298)의 항목 8 (readiness.md 생성)을 다음으로 교체합니다:

```markdown
8. **Readiness 데이터 생성**: JP1/JP2 Visual Summary에서 사용할 데이터를 `{output_base}/{feature_name}/readiness.md`에 저장합니다:

   **JP1 데이터** (specs-only 모드에서도 생성):
   - scenario_summaries: PRD User Journey에서 핵심 시나리오 3~5개를 1~2문장으로 축약합니다.
     각 시나리오에 관련 FR 번호를 태깅합니다.
   ...
   - side_effect_high_count: brownfield-context.md 영향 분석의 HIGH 위험도 항목 수
   - customer_impact_changes: brownfield side-effect를 고객 관점 문장으로 번역한 목록입니다.
...
```markdown
> **mode="specs-only"** 인 경우 Stage 1-2 + **JP1 Readiness 생성**을 실행한 뒤 종료합니다.
> JP1 Readiness는 Stage 2 완료 직후에 readiness.md의 JP1 데이터 항목을 생성합니다.
> (scenario_summaries, tracking_completeness, ai_inferred_count, side_effect_high_count, customer_impact_changes)
> scope_gate_summary는 /specs 호출 시 spec 단계 Scope Gate 결과만 포함합니다.
```
...
변경:
> **mode="specs-only"** 인 경우 Stage 1-2 + JP1 Readiness 생성을 실행한 뒤 종료합니다.
```
...
| auto-sprint Step 1~3 | auto-sprint.md | BMad 파이프라인 구조 변경 없음 |
| auto-sprint Conductor 역할 | auto-sprint.md | 구조 변경 없음 (용어만 치환) |
| auto-sprint 피드백 재실행 | auto-sprint.md | 구조 변경 없음 (용어만 치환 + JP2 라우팅 행 추가) |
...
8. **scope-gate.md** 고객 영향 열 + 고객 영향 요약 추가
...
### auto-sprint.md 검증
- CP/CP1/CP2/Checkpoint 문자열이 0건인지 확인합니다 (force_cp1_review 제외)
- Layer 0 자동 승인 로직이 완전히 제거되었는지 확인합니다
- JP1 Visual Summary가 서사(Section 1) + 추가 항목(Section 2) + 시스템 영향(Section 3) 구조인지 확인합니다
- JP2 피드백 라우팅에 "요구사항 재검토" 행이 있는지 확인합니다
- Advanced에 상세 매핑 테이블이 포함되었는지 확인합니다

### scope-gate.md 검증
- goals 빈 배열 시 PRD Success Criteria fallback 로직이 있는지 확인합니다
- product-brief 단계에서 goals가 비어있을 때 Stage 1 SKIP이 명시되었는지 확인합니다
- 출력 형식에 고객 영향 요약이 포함되었는지 확인합니다

### deliverable-generator.md 검증
- tracking_source 분기 로직이 Stage 2 시작에 있는지 확인합니다
- success-criteria 경로에서 BRIEF-N 매핑이 스킵되는지 확인합니다
- readiness.md에 scenario_summaries, customer_impact_changes 등 JP1 데이터가 포함되었는지 확인합니다
- specs-only 모드에서 JP1 Readiness가 생성되는지 명시되었는지 확인합니다

### 파일 간 일관성 검증
- auto-sprint.md의 JP1 데이터 소스가 deliverable-generator.md의 readiness.md 스키마와 일치하는지 확인합니다
- scope-gate.md의 고객 영향 요약이 auto-sprint.md JP1 Section 3에서 활용 가능한 형태인지 확인합니다
- tracking_source 필드명이 3개 파일에서 일관되게 사용되는지 확인합니다
