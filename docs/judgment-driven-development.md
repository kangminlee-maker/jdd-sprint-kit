...
- JP1 → 재생성 → JP1: PRD가 방향이 틀리면 전체 재생성
- JP2 → JP1 역방향 루프: 프로토타입을 보고 나서야 요구사항 오류를 발견하면
  PRD부터 재생성 (이것은 실패가 아니라 정상적인 발견 프로세스)
...
이 루프가 가능한 이유는 재생성 비용이 충분히 낮기 때문이다.
프로토타입을 보고 나서야 요구사항의 오류를 발견하는 것은
실패가 아니라 **구체적 결과물이 촉진한 정상적인 발견**이다.
