...
Sprint Kit은 BMad Method를 활용하여 기획 산출물을 생성하는 실행 레이어입니다.
...
Sprint Kit은 현재 BMad Method를 활용하며, 에이전트 경로와 포맷 가이드에서 BMad에 종속되어 있습니다. 그러나 Sprint Kit의 논리적 핵심(번역, 델타, 판단)은 BMad에 의존하지 않습니다. **기존 인간 지식의 형태**에 따라
...
> BMad 종속 분리 로드맵: [`reviews/translation-ontology-roadmap.md`](reviews/translation-ontology-roadmap.md) §1
...
**재생성 범위의 출처(provenance) 기준**: 재생성 범위는 피드백의 출처에 따라 정교하게 결정됩니다:
- 사용자 입력 원문에 이미 있었음 → 번역/생성 오류 → 해당 단계만 재생성
- 사용자 입력에 없었으나 추론 가능했음 → AI 추론 누락 → 해당 단계 + 하류 재생성
- 사용자 입력에 전혀 없는 새로운 요구 → 새로운 입력 → Brief 수정 후 파이프라인 재시작

출처 기반 분류가 재생성 비용을 더 정밀하게 추정할 수 있게 합니다.
...
이상적으로는 Brief 입력에서 바로 프로토타입(JP2)까지 도달하여 사용자가 산출물만 판단하면 됩니다. 그러나 JP1에는 JP2가 대체할 수 없는 **방향 검증(direction validation)** 고유 기능이 있습니다: 프로토타입에 없는 시나리오의 누락 감지, 요구사항 수준의 우선순위 판단, 고객 여정과의 정합성 확인은 JP2에서 수행할 수 없습니다. 또한 현재 AI 속도로는 JP2까지 한 번에 도달하는 데 수십 분이 걸리므로, JP1은 방향 검증이라는 고유 역할과 실용적 보완을 동시에 수행합니다. AI가 충분히 빨라져도 JP1의 형태는 변할 수 있으나(예: 프로토타입과 함께 요구사항 체크리스트 제시), 방향 검증 기능 자체는 남을 수 있습니다.
...
3. Bridge는 "두 시스템 간의 번역기"이지만, 동일 포맷을 공유하므로 번역이 불필요

**결론**: Sprint Kit은 BMad와 별개의 시스템이 아니라 BMad를 활용하는 실행 레이어이므로,
